...
class Database:
    def __init__(self):
        ...
        self.conn = mysql.connector.connect(host='localhost', password='montforT&2020', user='root',database="tkinter_project1")
    ...
    def insert_problem(self, name, difficulty, topic, status, url, notes):
        ...
        query = """INSERT INTO problems (name, difficulty, topic, status, url, notes, user_Id, embedding)  
                    values (%s, %s, %s, %s, %s, %s, %s, %s) """
        
        self.cursor.execute(query, (name, difficulty, topic, status, url, notes, user_Id, embedding))
        self.conn.commit()
        if(status=="Solved"):
    ...
    def semantic_search(self, text, top_k=10):
        ...
        userId = self.current_user_id()
        query = "SELECT name, embedding FROM problems where user_Id=%s"
        self.cursor.execute(query,(userId,))
        ...
        for name, emb_json in results:
            ...
            sim = np.dot(text_vec, emb) / (norm(text_vec) * norm(emb))
            similarities.append((name, sim))
        
        similarities.sort(key=lambda x: x[1], reverse=True)
        names = [name for name, score in similarities[:top_k]]
